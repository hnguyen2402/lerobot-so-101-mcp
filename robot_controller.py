...
class RobotController:
    ...
    def _connect_robot(self) -> None:
        ...
        keys_to_exclude = ["type", "remote_ip"]
        ...
        try:
            ...
            logger.error(f"Connect to {self.robot_type}: FAILED | {e}")
            raise
    ...
    def _connect_robot_readonly(self) -> None:
        ...
        try:
            ...
            logger.error(f"Connect to {self.robot_type} in READ-ONLY mode: FAILED | {e}")
            raise
    ...
    def _refresh_state(self) -> None:
        ...
        try:
            ...
            logger.error(f"Read Robot State: FAILED | {e}", exc_info=True)
    ...
    def _get_human_readable_state(self) -> Dict[str, float]:
        ...
        return {
            ...
            "gripper_openness_pct": self._deg_to_norm("gripper", positions_deg.get("gripper", 0.0)),
        }
    ...
    # henry
    def set_joints_norm(self, positions_norm: Dict[str, float], use_interpolation: bool = True) -> MoveResult:
        """Set joints to absolute positions."""
        positions_deg = {name: self._norm_to_deg(name, norm_val) for name, norm_val in positions_norm.items()}
        return self.set_joints_absolute(positions_deg, use_interpolation)
    ...
    def set_joints_absolute(self, positions_deg: Dict[str, float], use_interpolation: bool = True) -> MoveResult:
        ...
        try:
            ...
            logger.error(f"Move {self.robot_type}: FAILED | {e}", exc_info=True)
            self._refresh_state()
            return MoveResult(False, f"Move {self.robot_type}: FAILED | {e}", robot_state=self._get_full_state())
        
        return MoveResult(True, f"Move {self.robot_type}: SUCCESS", robot_state=self._get_full_state())
    ...
    def execute_intuitive_move(
        ...
        move_gripper_down_mm: Optional[float] = None,
        move_gripper_forward_mm: Optional[float] = None,
        move_gripper_backward_mm: Optional[float] = None,
        tilt_gripper_down_angle: Optional[float] = None,
        tilt_gripper_up_angle: Optional[float] = None,
        ...
        net_move_gripper_up_mm = 0.0
        if move_gripper_up_mm is not None:
            net_move_gripper_up_mm += move_gripper_up_mm
        if move_gripper_down_mm is not None:
            net_move_gripper_up_mm -= move_gripper_down_mm
        
        net_move_gripper_forward_mm = 0.0
        if move_gripper_forward_mm is not None:
            net_move_gripper_forward_mm += move_gripper_forward_mm
        if move_gripper_backward_mm is not None:
            net_move_gripper_forward_mm -= move_gripper_backward_mm
        
        net_tilt_gripper_down_angle = 0.0
        if tilt_gripper_down_angle is not None:
            net_tilt_gripper_down_angle += tilt_gripper_down_angle
        if tilt_gripper_up_angle is not None:
            net_tilt_gripper_down_angle -= tilt_gripper_up_angle
        
        net_rotate_gripper_ccw_angle = 0.0
        if rotate_gripper_counterclockwise_angle is not None:
            net_rotate_gripper_ccw_angle += rotate_gripper_counterclockwise_angle
        if rotate_gripper_clockwise_angle is not None:
            net_rotate_gripper_ccw_angle -= rotate_gripper_clockwise_angle
        
        net_rotate_robot_left_angle = 0.0
        if rotate_robot_left_angle is not None:
            net_rotate_robot_left_angle += rotate_robot_left_angle
        if rotate_robot_right_angle is not None:
            net_rotate_robot_left_angle -= rotate_robot_right_angle
            
        target_positions = self.positions_deg.copy()
        
        # Handle cartesian movements
        if net_move_gripper_up_mm != 0.0 or net_move_gripper_forward_mm != 0.0:
            target_x = self.cartesian_mm["x"] + net_move_gripper_forward_mm
            target_z = self.cartesian_mm["z"] + net_move_gripper_up_mm
        ...
        if net_tilt_gripper_down_angle != 0.0:
            target_positions["wrist_flex"] += net_tilt_gripper_down_angle
        if net_rotate_gripper_ccw_angle != 0.0:
            target_positions["wrist_roll"] -= net_rotate_gripper_ccw_angle
        if net_rotate_robot_left_angle != 0.0:
            target_positions["shoulder_pan"] -= net_rotate_robot_left_angle
    ...
    def get_camera_images(self, camera_names: list) -> Dict[str, np.ndarray]:
        ...
        try:
            ...
            logger.error(f"Get Camera Images {self.robot_type}: FAILED | {e}", exc_info=True)
            return {}
    ...
    def disconnect(self, reset_pos: bool = True) -> None:
        ...
        if reset_pos and not self.read_only:
            try:
                ...
                logger.error(f"Rest Position {self.robot_type}: FAILED | {e}", exc_info=True)
        ...
        try:
            ...
            logger.error(f"Disconnect {self.robot_type}: FAILED | {e}", exc_info=True)
        finally:
            self.robot = None
